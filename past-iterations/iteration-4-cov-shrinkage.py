...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 9 17:30:12 2026
...
==================
    ...
    ===================================================================
    EXPERIMENT COMPARISON (Mean-Variance combination)
    ===================================================================
                               Sample μ - Sample Σ Sample μ - Ledoit Σ
    Final Wealth                            $19.44              $20.74
    Avg Sharpe                               1.466               1.487
    Avg Excess Return vs SPY                 10.0%               10.5%
    Avg Excess Return vs Eq-Wt                6.6%                7.2%
    Win Rate vs SPY (Return)         12/15 (80.0%)       12/15 (80.0%)
    Win Rate vs Eq-Wt (Return)        8/15 (53.3%)        8/15 (53.3%)
    Win Rate vs SPY (Sharpe)          8/15 (53.3%)        7/15 (46.7%)
    Win Rate vs Eq-Wt (Sharpe)        7/15 (46.7%)        7/15 (46.7%)
    Max Drawdown                            -0.363              -0.359
    Avg Effective N                           14.0                15.2
    Avg Turnover                            181.9%              181.6%
    Avg Stocks at Cap                          4.1                 3.7

    
    === Final Value of $1.0 Invested ===
    Sample μ - Sample Σ : $19.44
    Sample μ - Ledoit Σ : $20.74
    Equal-Weighted      : $10.73
    SPY                 : $ 7.02
  
==================
Findings
==================
- Fixing the duration mismatch produced more realistic out-of-sample 
    performance estimates (previously inflated)

- Covariance shrinkage (using Ledoit-Wolf) produced modest improvements:
    - modestly increased diversification (Effective N from 14.0 -> 15.2)
    - slightly improved out-of-sample Sharpe (1.466 -> 1.487),
    - modestly improved final wealth ($19.44 -> $20.74),
    - slightly reduced drawdown (~44 bps).
    
    - But, turnover stayed virtually unchanged and incredibly high (~181%)

==================
Potential explanation
==================
    - Mean return estimation error potentially dominates MVO
    - Feeding raw historical sample means amplifies the estimation error
        thus, aggressively churning portfolio into sample noise regardless of
        covariance stability
    - This is seen in different years where multiple stocks hit the maximum
    weight cap.
    - Thus, maximum allocation bound itself is acting as a form of regularization
...
def load_dgs1_data (csv_path = 'DGS1.csv'):
    ...
    Returns:
        pd.DataFrame -
            index: daily dates,
            column: DGS1 (forward filled 1Y Treasury yields)
    """
...
def calculate_metrics(daily_returns: pd.Series,
    ...
    Compute annual performance metrics from daily returns and risk-free rate:
        ...
        - annualized volatility (scaled by trading days in a year)
        - Sharpe ratio
    ...
    Returns:
        dict[str, float]:
            ...
            - 'volatility': annual volatility value
            - 'sharpe': sharpe ratio
...
def simulate_portfolio(prices, weights, prev_weights,
    ...
    Returns:
        tuple: 
            ...
            turnover (float) - turnover of the optimized portfolio
    """
...
def effective_n(weights):
    ...
    Returns float - effective number of holdings
    """
...
def portfolio_optimizer(TRAIN_YEAR, TEST_YEAR, tickers,
                        ...
                        cov_shrinkage = False, verbose = False):
    """
    ...
    Returns:
        dict - metrics, updated_capital, updated weights, portfolio values,
            effective_n (optimized) and turnover (optimized)
    ...
    # conditional covariance matrix (sample vs shrinkage using Ledoit-Wolf)
    ...
    if verbose:
        ...
        print(f'Max weight (should be <= 10%): '
              f'{optimized_weights.max()*100:0.2f}%')
    ...
    return {
        ...
        'structure': {
            ...
            'num_stocks_at_bound': num_stocks_at_bound
            },
...
def run_walk_forward_backtest(start_year = 2010, end_year = 2024,
    ...
    summary = pd.DataFrame( [
        {
            ...
            'Stocks at Cap': r['structure']['num_stocks_at_bound']
    ...
    mean_label = "Sample μ" # add shrinkage later
    
    experiment_label = f"MVO | {mean_label} + {cov_label} | max w = {MAX_WEIGHT:.1%}"
...
def build_experiment_summary(experiments):
    ...
    Inputs:
        experiments (dict) produced by run_walk_forward_backtest():
